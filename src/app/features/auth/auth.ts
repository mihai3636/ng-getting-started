    ...
    <section>
      <div class="container login">
        <form (ngSubmit)="onSignIn()" class="form">
          <mat-card>
            <mat-card-content class="flow-content">
              <svg
                fill="#fff"
                version="1.1"
                id="Layer_1"
    ...
    this.authService.signIn(email, password).subscribe({
      ...
      error: (err) => {
        if (!(err instanceof FirebaseError)) {
          this.authError.set('Oops, something went wrong');
          return;
        }

        const fbError = err as FirebaseError;
        this.authError.set(fbError.message);
      },
    });
  }
}
